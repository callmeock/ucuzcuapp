...
function calcMarket(items: Product[], market: MarketName) {
  let total = 0
...
export default function BasketPanel({ open, onClose, basket, onRemove }: BasketPanelProps) {
      ...
      <div className={`fixed top-14 right-0 bottom-0 w-96 bg-white shadow-2xl z-50 flex flex-col transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="bg-primary text-white px-4 py-3 flex justify-between items-center shrink-0">
          <h3 className="font-bold text-base">🛒 Sepetim
            {items.length > 0 && <span className="ml-2 bg-white/20 text-sm px-2 py-0.5 rounded-full">{items.length} ürün</span>}
          </h3>
              ...
              {items.map((p) => {
                ...
                return (
                  <div key={p.id} className="flex items-center gap-3 px-4 py-2.5">
                    ...
                    {minPrice && <span className="text-sm font-bold text-green-600 shrink-0">{minPrice.toFixed(2)} ₺</span>}
                    <button onClick={() => onRemove(p.id)} className="text-gray-300 hover:text-red-400 transition-colors text-lg shrink-0">✕</button>
                  </div>
                )
              })}
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 pb-3">
              ...
              {view === 'cheapest' && (
                <div className="space-y-1">
                  ...
                  {cheapest.rows.map(({ product, market, price }) => (
                    <div key={product.id} className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2.5">
                      ...
                      <span className={`font-bold text-sm shrink-0 ${price ? 'text-green-600' : 'text-gray-300'}`}>
                  ...
                  <div className="mt-3 bg-amber-50 border-2 border-amber-200 rounded-xl px-4 py-3">
                    ...
                    {(() => {
                      ...
                      cheapest.rows.forEach(({ market, price }) => {
                        if (market && price) byMarket[market] = (byMarket[market] || 0) + price
                      })
              ...
              {MARKETS.includes(view as MarketName) && (() => {
                ...
                return (
                  <div className="space-y-1">
                    ...
                    {rows.map(({ product, price, available: avail }) => (
                      <div key={product.id}
                    ...
                    <div className="mt-3 rounded-xl px-4 py-3 border-2" style={{ borderColor: color + '66', background: color + '11' }}>
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          {unavailableCount > 0
                            ? <div className="text-xs text-red-500 mt-0.5">{items.length - unavailableCount}/{items.length} ürün mevcut</div>
