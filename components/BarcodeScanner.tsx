...
/** Açık video önizlemesinden kare al → JPEG File */
function grabLiveFrame(): Promise<File | null> {
...
async function decodeImageFile(file: File): Promise<string> {
  const tmpId = 'ucuzcu-barcode-file-tmp'
  ...
  const scanner = new Html5Qrcode(tmpId, { verbose: false, formatsToSupport: FORMATS })
  try {
...
export default function BarcodeScanner({ onDetected, onClose }: BarcodeScannerProps) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        let facing = ''
        try { facing = scanner.getRunningTrackSettings()?.facingMode || '' } catch { /* ignore */ }
        if (facing === 'user') {
        ...
        for (const fb of fallbacks) {
          try {
            if (scanner.isScanning) { try { await scanner.stop() } catch { /* ignore */ } }
            await startWith(fb)
            started = true
            break
          } catch { /* next */ }
        ...
        if (!cancelled) {
          if (started) {
            ...
            setErrorMsg((err as { message?: string }).message || 'Arka kamera açılamadı')
            setStatus('error')
          }
        }
      }
  ...
  const toggleTorch = async () => {
    ...
    try {
      const torch = s.getRunningTrackCameraCapabilities().torchFeature()
  ...
  /** Canlı önizlemeden kare yakala ve oku — native kamera app'e gitmez */
  const captureAndRead = async () => {
    if (busy || doneRef.current) return
    setBusy(true)
    showToast('Yakalanıyor…')
    try {
      const file = await grabLiveFrame()
      if (!file) {
        showToast('Görüntü alınamadı — biraz bekle')
        return
      }
      showToast('Barkod okunuyor…')
      const text = await decodeImageFile(file)
      await finish(text)
    } catch {
      showToast('Okunamadı — hizala ve tekrar yakala')
    } finally {
      setBusy(false)
    }
  }

  /** Sadece galeri — capture yok, kamera app açılmaz */
  const onGallery = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast('Fotoğrafta barkod yok — net çekilmiş olsun')
    } finally {
      setBusy(false)
    }
  }
      ...
      <div className="relative flex-1 min-h-0 bg-black">
        ...
        {toast && (
          <div className="absolute top-4 inset-x-0 z-30 flex justify-center pointer-events-none">
            <p className="text-sm text-white bg-black/70 rounded-full px-4 py-2">{toast}</p>
          </div>
        )}
      </div>

      <div className="relative z-20 bg-gray-950 px-4 pt-4 pb-4 safe-bottom space-y-3 border-t border-white/10">
        {/* Ana aksiyon: canlı kareyi yakala */}
        <button
          type="button"
          disabled={status !== 'ready' || busy}
          onClick={() => void captureAndRead()}
          className="w-full flex flex-col items-center justify-center gap-1 bg-green-500 hover:bg-green-400 disabled:bg-white/10 disabled:text-white/30 text-white rounded-2xl py-4 font-bold active:scale-[0.98] transition"
        >
          <span className="flex items-center gap-2 text-base">
            <span className="w-3 h-3 rounded-full bg-white/90" />
            {busy ? 'Okunuyor…' : 'Şimdi yakala'}
          </span>
          <span className="text-[11px] font-normal text-white/80">Açık kameradan kare al — uygulama dışına çıkmaz</span>
        </button>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 bg-white/10 text-white rounded-xl py-3 text-sm font-semibold disabled:opacity-50"
          >
            Galeriden seç
          </button>
          <button
            ...
            className="flex items-center justify-center gap-2 bg-red-600 hover:bg-red-500 text-white rounded-xl py-3 text-sm font-semibold disabled:opacity-50"
          >
            Demo kola
          </button>
        </div>
        ...
        >
          ...
          <button
            ...
            className="bg-white/15 disabled:bg-white/5 disabled:text-white/30 text-white px-5 rounded-xl font-semibold text-sm"
          >
            Ara
          </button>
        </form>
